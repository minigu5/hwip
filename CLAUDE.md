...
- **`$$` 없는 LaTeX 감지 → 강제 변환 경고** (`#noDelimiterModal`): 일부 AI가 `\frac{dp}{dx}` 같은 LaTeX를 `$$` 구분자 없이 출력하는 경우가 있다. `app.js`의 `isLikelyUndelimitedLatex()`가 `\[a-zA-Z]{2,}` 패턴으로 감지해 모달을 띄운다. 변환은 강제로 진행되며(`converter.js`가 구분자 없이도 처리 가능), 모달에는 AI에게 `$$` 기호로 감싸서 다시 작성해 달라는 프롬프트 복사 버튼이 포함된다.
